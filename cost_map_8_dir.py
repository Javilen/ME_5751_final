		...
		while len(q) > 0 and distance != -1:
			...
			for [i,j] in ([x+1,y],[x-1,y],[x,y+1],[x,y-1],[x+1,y+1],[x-1,y+1],[x-1,y-1]):  # can modify for 8 directions here  for [i,j] in ([x+1,y],[x-1,y],[x,y+1],[x,y-1]):
				...
				if i >=0 and i < L and j >=0 and j < L and gridImage[i][j] == 0:  
					...
					if [i,j] == [x+1,y+1] or [i,j] == [x+1,y-1] or [i,j] == [x-1,y+1] or [i,j] == [x-1,y-1]:
						q.append([i,j,depth+math.sqrt(2)])
						brushMap[i][j]=depth+math.sqrt(2)
					else:
				...
				if i >=0 and i < L and j >=0 and j < L and gridImage[i][j] != 0 and grid[i][j] != 1: #see if we are too close to another 
					if brushMap[i][j] > (depth+1) and ([i,j]==[x+1,y] or [i,j]==[x-1,y] or [i,j]==[x,y+1] or [i,j]==[x,y-1]):  # keep the smaller one for brushfire mission statement
						brushMap[i][j] = (depth+1)
					if brushMap[i][j] > (depth+math.sqrt(2)) and ([i,j]==[x+1,y+1] or [i,j]==[x-1,y+1] or [i,j]==[x+1,y+1] or [i,j]==[x-1,y-1]):  # keep the smaller one for brushfire mission statement
						brushMap[i][j] = (depth+math.sqrt(2))
